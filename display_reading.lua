...
function ReadingDisplay:create(weather_lockscreen, weather_data)
    ...
    local title_font_size = math.floor(24 * scale_factor)
    local author_font_size = math.floor(18 * scale_factor)
    local progress_font_size = math.floor(16 * scale_factor)
    local temp_font_size = math.floor(32 * scale_factor)
    local weather_detail_font_size = math.floor(16 * scale_factor)
    local location_font_size = math.floor(14 * scale_factor)
    local weather_icon_size = math.floor(60 * scale_factor)
    
    local card_padding = math.floor(25 * scale_factor)
    local spacing = math.floor(15 * scale_factor)
    local small_spacing = math.floor(8 * scale_factor)
    ...
    -- Overlay card at bottom with frosted background effect
    local card_width = math.floor(screen_width * 0.9)
    local card_max_width = math.floor(800 * scale_factor)
    ...
    -- Left: Page numbers (aligned with title)
    local page_text = ""
    ...
    local card_container = BottomContainer:new{
        dimen = Geom:new{ w = screen_width, h = screen_height },
        CenterContainer:new{
            dimen = Geom:new{ w = screen_width, h = card:getSize().h + math.floor(40 * scale_factor) },
            card,
        }
    }
